...
func TestBuyAsset(t *testing.T) {
	...
	investorAssetPosition := NewInvestorAssetPosition("asset1", 15)
	...
	// add buy order
	wg.Add(1)
	order := NewOrder("1", investor, asset1, 5, 1, 5, "SELL")
	orderChan <- order

	// add sell order

	order2 := NewOrder("2", investor2, asset1, 5, 1, 5, "BUY")
	orderChan <- order2
	wg.Wait()

	assert := assert.New(t)
	assert.Equal("CLOSED", order.Status, "Order 1 should be closed")
	assert.Equal(0, order.PendigShares, "Order 1 should have 0 PendingShares")
	assert.Equal("CLOSED", order2.Status, "Order 2 should be closed")
	assert.Equal(0, order2.PendigShares, "Order 2 should have 0 PendingShares")
...
func TestBuyAssetWithDifferentAssents(t *testing.T) {
	...
	order2 := NewOrder("2", investor2, asset2, 5, 1, 5, "BUY")
	orderChan <- order2

	assert := assert.New(t)
	assert.Equal("OPEN", order.Status, "Order 1 should be closed")
	assert.Equal(5, order.PendigShares, "Order 1 should have 5 PendingShares")
	assert.Equal("OPEN", order2.Status, "Order 2 should be closed")
	assert.Equal(5, order2.PendigShares, "Order 2 should have 5 PendingShares")
}
...
func TestBuyPartialAsset(t *testing.T) {
	...
	order2 := NewOrder("1", investor2, asset1, 5, 1, 5.0, "BUY")
	orderChan <- order2

	// investidor 1 quer vender 3 shares
	order := NewOrder("2", investor, asset1, 3, 1, 5.0, "SELL")
	orderChan <- order
	...
	assert.Equal(2, order2.PendigShares, "Order 2 should have 2 PendingShares")
	...
	wg.Add(1)
	order3 := NewOrder("3", investor3, asset1, 2, 1, 5.0, "SELL")
	orderChan <- order3
	wg.Wait()

	assert.Equal("CLOSED", order3.Status, "Order 3 should be closed")
	assert.Equal(0, order3.PendigShares, "Order 3 should have 0 PendingShares")
...
func TestBuyWithDifferentPrice(t *testing.T) {
	...
	wg.Add(1)
	// investidor 2 quer comprar 5 shares
	order2 := NewOrder("2", investor2, asset1, 5, 0, 5.0, "BUY")
	orderChan <- order2

	// investidor 1 quer vender 3 shares
	order := NewOrder("1", investor, asset1, 3, 0, 4.0, "SELL")
	orderChan <- order
	...
	wg.Add(1)
	order3 := NewOrder("3", investor3, asset1, 3, 0, 4.5, "SELL")
	orderChan <- order3

	wg.Wait()

	assert.Equal("OPEN", order3.Status, "Order 3 should be open")
	assert.Equal(1, order3.PendigShares, "Order 3 should have 1 PendingShares")
...
func TestNoMatch(t *testing.T) {
	...
	wg.Add(0)
	// investidor 1 quer vender 3 shares
	order := NewOrder("1", investor, asset1, 3, 1, 6.0, "SELL")
	orderChan <- order

	// investidor 2 quer comprar 5 shares
	order2 := NewOrder("2", investor2, asset1, 5, 1, 5.0, "BUY")
	orderChan <- order2
	...
	assert.Equal("OPEN", order.Status, "Order 1 should be closed")
	assert.Equal("OPEN", order2.Status, "Order 2 should be OPEN")
	assert.Equal(3, order.PendigShares, "Order 1 should have 3 PendingShares")
	assert.Equal(5, order2.PendigShares, "Order 2 should have 5 PendingShares")
}
